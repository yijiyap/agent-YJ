...
import { useNuxt, resolveFiles, defineNuxtModule, addTemplate, addPlugin, addComponent, updateTemplates, addVitePlugin, addWebpackPlugin, addBuildPlugin, findPath, addImportsSources, tryResolveModule, isIgnored, resolveAlias, addPluginTemplate, logger, normalizeModuleTranspilePath, resolveIgnorePatterns, createResolver, nuxtCtx, resolvePath as resolvePath$1, installModule, useNitro, loadNuxtConfig, normalizeTemplate, compileTemplate, normalizePlugin, templateUtils } from '@nuxt/kit';
import escapeRE from 'escape-string-regexp';
import fse from 'fs-extra';
import { parseURL, parseQuery, encodePath, joinURL, withTrailingSlash, withoutLeadingSlash } from 'ufo';
import fs, { existsSync, readdirSync, statSync, readFileSync, promises, mkdirSync, writeFileSync } from 'node:fs';
...
async function generateRoutesFromFiles(files, pagesDir, shouldExtractBuildMeta = false, vfs) {
  const routes = [];
...
function warnAboutDuplicateComponent(componentName, filePath, duplicatePath) {
  console.warn(
    `[nuxt] Two component files resolving to the same name \`${componentName}\`:
...
async function initNitro(nuxt) {
  ...
  (_a = nitro.options._config).storage || (_a.storage = {});
      ...
      walk(this.parse(code), {
        enter(node) {
          if (node.type === "ImportDeclaration" && node.source.value === "vue") {
            importName = importName ?? node.specifiers.find((s2) => s2.type === "ImportSpecifier" && s2.imported.name === "defineComponent")?.local.name;
          }
...
async function loadNuxt(opts) {
  ...
  if (options.builder === "@nuxt/webpack-builder") {
    if (!await import('./chunks/features.mjs').then((r) => r.ensurePackageInstalled(options.rootDir, "@nuxt/webpack-builder", options.modulesDir))) {
